...
def kmeans_m():
    """
    Description: K-means is a centroid-based clustering algorithm that partitions data into K clusters by
minimizing the inertia (sum of squared distances between points and their cluster centroids).
Steps:
...
def help_m():
    """
    all imports:
    vocab_m - short description of main terms
    one_m - basic 
...
def dist_m():
    """
    realisation of euclidian distance: 
...
def isofor_m():
    """
    Isolation Forest
Description: Isolation Forest is an anomaly detection algorithm that isolates outliers by randomly
partitioning the data. Outliers are isolated in fewer steps compared to inliers.
Steps:
1. Build isolation trees by randomly selecting features and splitting points.
2. Calculate the anomaly score for each point based on the average path length to isolation.
3. Points with higher anomaly scores are considered outliers.
